...
## 19. Scaling the wheel: 12-16 sensor pads

The bench prototype has two MAX30102 modules behind one TCA9548A multiplexer
(Andrew's wiring drawing, revision A, 2026-09-22). The wheel build puts 12-16
pads around the rim so a hand finds one wherever it grips. This section is the
requirement list for that step; none of it is built yet.

**Only one pad streams at a time.** The firmware scans the pads for skin
contact, streams the one with the best signal, keeps a second warmed up as the
standby, and hands over when the first hand lets go. Everything downstream --
the packet, the relay, the app, the database, the website -- keeps working
unchanged, because it still receives one 1 s packet per second.

That choice is what makes 16 pads affordable. Streaming them all would be
about 7.6 kB/s instead of 472 B/s and would force a new packet format, a new
database schema and a new upload path.

### Electrical

| Item | Bench now | 12-16 pads |
|---|---|---|
| Multiplexers | one TCA9548A at `0x70` | two (8 channels each). Addresses are set by pins A0-A2, so `0x70` and `0x71`; up to 8 are possible |
| I²C pull-ups | the modules' own | one pair per active branch. Wheel-length wiring adds capacitance: check the SCL/SDA rise time on a scope before trusting the bus |
| 5 V supply | 2 modules | measure one module's current at the LED brightness the firmware sets, multiply by the pad count, add margin, then size the converter. Do not take a number from a web page: the draw depends on brightness and pulse width |
| Harness | jumpers | a ring harness, with a plan for crossing the steering column (slip ring or a wire loop with enough slack for full lock-to-lock) |
| Placement | two fixed points | the pads must sit where hands actually rest; see the contact rate measured per pad (below) |

### Firmware

- **Scan.** Switch the multiplexer to each pad in turn and read its IR DC
  level. Contact is the same test the firmware already uses (§3): the IR mean
  above the finger threshold, which on our module is well above the ~50 000
  counts a resting object gives.
- **Choose.** Among pads with contact, stream the one with the best signal
  quality, not simply the first found. The phone already computes a quality
  score per second (§4); the firmware's cheap equivalent is its periodicity
  `quality` value.
- **Standby.** Keep a second pad selected and sampling so a handover does not
  restart the 4 s estimator window from nothing.
- **Handover.** When the streaming pad loses contact, switch to the standby
  and promote the next candidate. Mark the packet so the receiver knows the
  source changed.
- **Bus timing.** Do not switch the multiplexer for every sample. Each module
  buffers samples in its own FIFO (32 deep), so read a burst per pad. A
  switch-per-sample schedule wastes most of the I²C bandwidth on overhead.

### Packet

No new version is needed. The v3 header's `flags` byte has four unused bits
(`0x10`-`0x80`), which hold a pad index 0-15. Packets from the current
one-sensor firmware carry zeros there, so old captures stay valid and decoders
that ignore the field keep working. Whatever is decided, change it in
`system/common/ppg_protocol.py` and the copies, `ppg_frame.h`, and
`mobile-app/lib/ble/protocol.ts` together -- the tests compare them
byte-for-byte (§14).

### What each pad count buys

Report contact rate per pad from a real drive (the app already stores a
`finger` flag per second, §8), then add pads where hands actually rest. 16
pads with poor placement are worse than 8 placed from measured data.

### Drawing it

The prototype drawing is a picture, not an electrical design: no netlist, no
rule checks, no parts list. For 12-16 repeated pads, redraw it in **KiCad**
(free, and what reviewers expect): one sheet reused per pad, electrical rule
checks, and a parts list generated from the schematic. A drawing tool
(draw.io, Figma) is fine for the report figure, but keep the electrical truth
in KiCad.
